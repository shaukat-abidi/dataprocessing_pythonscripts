...
filepath_savemodel = 'C://Shaukat//code//rainfall//model_stored/orford//model_lstm_orford_DONTUSE.h5'
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=10, verbose=1, mode='auto', epsilon=0.0001, cooldown=0, min_lr=0.001)

# history = model.fit(x_train, y_train, nb_epoch=500, verbose=1, validation_data=(x_test, y_test), batch_size = train_points, callbacks=[checkpointer])
history = model.fit(x_train, y_train, nb_epoch=10, verbose=1, validation_data=(x_test, y_test), batch_size = train_points, callbacks=[checkpointer, reduce_lr])
